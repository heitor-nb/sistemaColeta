...
try
{
    ...
    do
    {
        ...
        for (int i = 0; i < caminhoes; i++)
        {
            ...
            var task = Task.Run(() =>
            {
                grafo.Percurso(alfabeto[localI], 0, 5, visitados, carrocinhas); // !! problema do símbolo errado
            });
    ...
    Console.WriteLine("----- Descobre a quantidade de funcionários: -----");

    int funcionarios = 5;

    while(tempo < 480 && funcionarios > 2)
    {
        // A partir da segunda iteração do while, é necessário resetar as
        // informações dos nós, pois essas são alteradas durante o percuso.
        if (caminhoes != 0)
        ...
        for (int i = 0; i < caminhoes; i++)
        {
            ...
            var task = Task.Run(() =>
            {
                grafo.Percurso(alfabeto[localI], 0, funcionarios, visitados, carrocinhas); // !! problema do símbolo errado
            });
